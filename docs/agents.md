...
Built-in adapters run in a temporary evidence directory and restrict agent tools where supported. They do not check out fork code or load its instruction files. Codex ignores user configuration and execution rules, disables shell tools, apps, plugins and web search, and retains its read-only sandbox. Authentication still uses the existing Codex login; set model and effort in Forkpicker because Codex's user configuration is not loaded. This requires a Codex version supporting `exec --ignore-user-config`. Claude uses `--safe-mode`, disables built-in tools and explicitly excludes inherited MCP servers and skills. Other adapters and custom commands can inherit additional capabilities from their CLI configuration; Forkpicker does not provide an OS sandbox for arbitrary wrappers. Previously exported profiles replace these defaults and need updating to receive these restrictions.
